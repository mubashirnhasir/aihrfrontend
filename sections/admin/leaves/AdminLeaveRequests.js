...
const AdminLeaveRequests = () => {
  ...
  const fetchLeaveRequests = async (silent = false) => {
    try {
      ...
      if (response.ok) {
        ...
        if (JSON.stringify(newRequests) !== JSON.stringify(leaveRequests) || !silent) {
  ...
  const handleLeaveAction = async (requestId, action, rejectionReason = '') => {
    try {
      ...
      const response = await fetch(`http://localhost:5000/api/leaves/request/${requestId}/status`, {
        method: 'PUT',
        headers: {
          'Authorization': `Bearer ${token}`,
          'Content-Type': 'application/json'
        ...
        showSuccess(`Leave request ${actionText} successfully! Employee will see updates shortly.`, 4000);
        
      } else {
        const errorData = await response.json();
        const errorMessage = errorData.message || `Failed to ${action} leave request`;
  ...
  return (
    <div className="space-y-6">
      ...
      <div className="flex justify-between items-center">
        <div>
          <h2 className="text-xl font-semibold text-gray-900">Leave Requests Management</h2>
      ...
      <div className="border-b border-gray-200">
        <nav className="flex space-x-8">
          {[
            { key: 'pending', label: 'Pending Approval', count: leaveRequests.filter(r => r.status === 'pending').length },
      ...
      {leaveRequests.length === 0 ? (
        <div className="text-center py-12">
          ...
          <p className="mt-1 text-sm text-gray-500">
            {filter === 'pending' ? 'No pending requests at this time.' : `No ${filter} requests found.`}
          </p>
        </div>
      ) : (
                    ...
                    <div className="flex items-center">
                      ...
                      <div className="ml-4">
                        <div className="flex items-center">
                          ...
                          <span className={`ml-2 inline-flex px-2 py-1 text-xs font-medium rounded-full ${getLeaveTypeColor(request.leaveType)}`}>
                        ...
                        <div className="mt-1 flex items-center text-sm text-gray-500">
                          ...
                          {request.isHalfDay && (
                            <>
                              <span className="mx-2">•</span>
                              <span className="text-orange-600">Half Day ({request.halfDayType})</span>
                            </>
                          )}
                        </div>
                        <div className="mt-1 text-sm text-gray-500">
                          <span className="font-medium">Department:</span> {request.employeeId?.department || 'N/A'}
                        </div>
                      </div>
                    </div>
                ...
                <div className="sm:flex sm:items-start">
                  <div className="mt-3 text-center sm:mt-0 sm:text-left w-full">
                    ...
                    <div className="space-y-3">
                      <div>
                        <span className="font-medium">Employee:</span> {selectedRequest.employeeId?.name}
                      </div>
                      <div>
                        <span className="font-medium">Department:</span> {selectedRequest.employeeId?.department}
                      </div>
                      <div>
                        <span className="font-medium">Leave Type:</span> {selectedRequest.leaveType}
                      </div>
                      <div>
                        <span className="font-medium">Duration:</span> {selectedRequest.duration} day(s)
                      </div>
                      <div>
                        <span className="font-medium">Dates:</span> {formatDate(selectedRequest.startDate)} - {formatDate(selectedRequest.endDate)}
                      </div>
                      {selectedRequest.isHalfDay && (
                        <div>
                          <span className="font-medium">Half Day:</span> {selectedRequest.halfDayType}
                        </div>
                      )}
                      <div>
                      ...
                      {selectedRequest.emergencyContact && (
                        <div>
                          <span className="font-medium">Emergency Contact:</span> {selectedRequest.emergencyContact}
                        </div>
                      )}
                      <div>
                        <span className="font-medium">Applied On:</span> {formatDate(selectedRequest.createdAt)}
                      </div>
                      <div>
