...
const EmployeeLeaveStatus = ({ 
  ...
  const recentRequests = leaveRequests
    .sort((a, b) => new Date(b.createdAt || b.appliedOn || b.startDate) - new Date(a.createdAt || a.appliedOn || a.startDate))
                ...
                <div className="flex items-start justify-between">
                  <div className="flex-1">
                    <div className="flex items-center space-x-2 mb-2">
                      ...
                      <span className={`inline-flex px-2 py-1 text-xs font-medium rounded-full ${getTypeColor(request.type)}`}>
                        {request.type?.charAt(0).toUpperCase() + request.type?.slice(1)} Leave
                      </span>
                      <span className="text-sm text-gray-600">
                        {request.duration || 1} day{(request.duration || 1) !== 1 ? 's' : ''}
                      </span>
                    </div>
                    ...
                    <div className="text-xs text-gray-500">
                      Applied on {formatDate(request.appliedOn || request.createdAt || request.startDate)}
                    </div>
                  </div>
