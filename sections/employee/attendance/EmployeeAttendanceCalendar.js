...
export default function EmployeeAttendanceCalendar({
  ...
  const [selectedDate, setSelectedDate] = useState(null); // Debug: Log the calendar data to see its structure
  useEffect(() => {
    console.log("Calendar Data:", calendarData);
  }, [calendarData]);

  // Mock data for testing colors - remove this after testing
  const mockCalendarData = {
    "2024-01-15": { status: "present", isLate: false, isEarly: false },
    "2024-01-16": { status: "present", isLate: true, isEarly: false },
    "2024-01-17": { status: "absent" },
    "2024-01-18": { status: "leave" },
    "2024-01-19": { status: "holiday" },
    "2024-01-20": { status: "present", isLate: false, isEarly: true },
  };
  ...
  const getAttendanceForDate = (day) => {
    if (!dataToUse || !day) return null;

    const dateKey = `${currentDate.getFullYear()}-${String(
      currentDate.getMonth() + 1
    ).padStart(2, "0")}-${String(day).padStart(2, "0")}`;
    return dataToUse[dateKey] || null;
  };
