...
export default function InvoiceSection() {
  ...
  return (
    <section className="p-6">
      {invoices.length === 0 ? (
        <div className="text-center py-8">
          <div className="text-gray-500">No invoices found. Create your first invoice!</div>
        </div>
      ) : (
        <div className="grid grid-cols-1 sm:grid-cols-2 md:grid-cols-3 lg:grid-cols-4 gap-6">
          {invoices.map((invoice) => (
            ...
            >
              {/* Status Badge */}
              <span className={`absolute top-4 right-4 text-xs font-medium px-2 py-1 rounded-full ${getStatusColor(invoice.status)}`}>
