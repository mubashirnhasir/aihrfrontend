...
export default function SharePanel({ jobDescription, formData }) {
  ...
  const handleCopy = async (content) => {
    try {
      ...
      setCopySuccess(
        `Copied ${platforms.find((p) => p.id === activeTab)?.name} version!`
      );
                ...
                >
                  <div className="flex items-start">
                    ...
                    <div className="ml-3 flex-1">
                      ...
                      <div className="text-gray-700 text-sm mt-1">{tweet}</div>
                    </div>
                  </div>
                </div>
              ))}
      ...
      <div className="p-6">
        ...
        <div className="flex gap-3 pt-4 border-t">
          ...
          {activeTab === "website" && (
            <button
              onClick={() => {
                const blob = new Blob([getCurrentContent()], {
                  type: "text/plain",
                });
                const url = URL.createObjectURL(blob);
                const a = document.createElement("a");
                a.href = url;
                a.download = `${formData.jobTitle.replace(
                  /\s+/g,
                  "_"
      ...
      <div className="px-6 pb-6">
        <details className="group">
          <summary className="cursor-pointer text-sm text-gray-500 hover:text-gray-700">
            View raw content for{" "}
            {platforms.find((p) => p.id === activeTab)?.name}
          </summary>
