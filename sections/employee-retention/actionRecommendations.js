...
export default function ActionRecommendations({ recommendations = [], analytics = null }) {
  ...
  const smartRecommendations = useMemo(() => {
    ...
    if (analytics.retentionTrends?.trend === 'declining') {
      additionalRecs.push({
        ...
        timeline: '2 weeks',
        category: 'Strategic',
        description: 'Develop and implement a company-wide retention strategy addressing the declining retention trend.',
        steps: [
    ...
    const riskDepts = analytics.departmentAnalytics?.filter(dept => 
      (dept.highRisk / dept.totalEmployees) > 0.2
    ) || [];
  ...
  return (
    <div className="space-y-6">
        ...
        <div className="bg-white rounded-lg shadow-sm border p-6">
          <div className="flex items-center justify-between">
            <div>
              ...
              <p className="text-2xl font-bold text-red-600">{groupedRecommendations.High.length}</p>
            </div>
            <div className="text-3xl">🚨</div>
          </div>
        </div>
        ...
        <div className="bg-white rounded-lg shadow-sm border p-6">
          <div className="flex items-center justify-between">
            <div>
              ...
              <p className="text-2xl font-bold text-green-600">{completedActions.size}</p>
            </div>
            <div className="text-3xl">✅</div>
          </div>
        </div>
      ...
      {Object.entries(groupedRecommendations).map(([priority, recs]) => {
        ...
        return (
          <div key={priority} className="bg-white rounded-lg shadow-sm border p-6">
                ...
                return (
                  <div 
                    ...
                    className={`border-2 rounded-lg p-4 transition-all duration-200 ${
                      ...
                      selectedAction === globalIndex ? 'ring-2 ring-blue-500' : ''
                    }`}
                  >
                    <div className="flex items-start justify-between">
                      <div className="flex-1">
                        ...
                        <div className="grid grid-cols-1 md:grid-cols-3 gap-4 mb-3">
                          <div>
                            ...
                            <div className="text-sm font-medium text-gray-900">{rec.department}</div>
                          </div>
                          <div>
                          ...
                          <div>
                            ...
                            <div className="text-sm font-medium text-gray-900">{rec.expectedImpact}</div>
                          </div>
                        </div>
                      ...
                      >
                        {selectedAction === globalIndex ? 'Hide Details' : 'View Details'}
                      </button>
                    </div>
        ...
        <div className="space-y-4">
          ...
          <div className="w-full bg-gray-200 rounded-full h-2">
            <div 
              ...
              style={{ 
                width: `${smartRecommendations.length > 0 ? (completedActions.size / smartRecommendations.length) * 100 : 0}%` 
              }}
            ></div>
          </div>
            ...
            <div>
              ...
              <div className="text-lg font-semibold text-red-600">
                {groupedRecommendations.High.filter((_, i) => 
                  completedActions.has(smartRecommendations.indexOf(groupedRecommendations.High[i]))
                ).length} / {groupedRecommendations.High.length}
              </div>
            </div>
            <div>
              ...
              <div className="text-lg font-semibold text-yellow-600">
                {groupedRecommendations.Medium.filter((_, i) => 
                  completedActions.has(smartRecommendations.indexOf(groupedRecommendations.Medium[i]))
                ).length} / {groupedRecommendations.Medium.length}
              </div>
            </div>
            <div>
              ...
              <div className="text-lg font-semibold text-green-600">
                {groupedRecommendations.Low.filter((_, i) => 
                  completedActions.has(smartRecommendations.indexOf(groupedRecommendations.Low[i]))
                ).length} / {groupedRecommendations.Low.length}
              </div>
            </div>
          </div>
        </div>
      ...
      <div className="bg-blue-50 rounded-lg border border-blue-200 p-6">
        ...
        <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
          <div className="bg-white rounded-lg p-4">
            <h4 className="font-medium text-gray-900 mb-2">📞 Schedule Stay Interviews</h4>
          ...
          <div className="bg-white rounded-lg p-4">
            <h4 className="font-medium text-gray-900 mb-2">📊 Review Team Satisfaction</h4>
            <p className="text-sm text-gray-600 mb-3">
          ...
          <div className="bg-white rounded-lg p-4">
            <h4 className="font-medium text-gray-900 mb-2">🎯 Set Retention Goals</h4>
            <p className="text-sm text-gray-600 mb-3">
