...
export default function DepartmentAnalytics({ departmentData = [], employees = [] }) {
  ...
  const departmentStats = useMemo(() => {
    ...
    return calculatedStats.map(dept => {
      const existingData = departmentData.find(d => d.department === dept.department);
      return {
        ...dept,
        retentionRate: existingData?.retentionRate || 
        ...
        {departmentStats.map((dept, index) => {
          ...
          return (
            ...
            >
              ...
              <div className="space-y-3">
                ...
                <div className="flex justify-between items-center">
                  ...
                  <span className="font-semibold text-red-600">
                    {dept.highRisk + dept.mediumRisk} ({riskPercentage.toFixed(1)}%)
                  </span>
                </div>
              </div>
              ...
              <div className="mt-4">
                <div className="text-xs text-gray-600 mb-1">Risk Distribution</div>
            ...
            <div>
              ...
              <div className="space-y-4">
                ...
                <div className="bg-blue-50 rounded-lg p-4">
                  <h5 className="font-medium text-blue-900 mb-3">Average Satisfaction Scores</h5>
                  <div className="space-y-2">
                    {[
                      { 
                        label: 'Job Satisfaction', 
                        value: selectedDeptEmployees.reduce((sum, emp) => sum + (emp.jobSatisfaction || 0), 0) / selectedDeptEmployees.length 
                      },
                      { 
                        label: 'Engagement Level', 
                        value: selectedDeptEmployees.reduce((sum, emp) => sum + (emp.engagementLevel || 0), 0) / selectedDeptEmployees.length 
                      },
                      { 
                        label: 'Work-Life Balance', 
                        value: selectedDeptEmployees.reduce((sum, emp) => sum + (emp.workLifeBalance || 0), 0) / selectedDeptEmployees.length 
                      },
                      { 
                        label: 'Career Growth', 
                        value: selectedDeptEmployees.reduce((sum, emp) => sum + (emp.careerGrowth || 0), 0) / selectedDeptEmployees.length 
                      }
                    ].map((item, idx) => (
                ...
                <div className="bg-yellow-50 rounded-lg p-4">
                  ...
                  <div className="space-y-2">
                    {(() => {
                      const deptData = departmentStats.find(d => d.department === selectedDepartment);
                      const riskLevel = ((deptData.highRisk + deptData.mediumRisk) / deptData.totalEmployees) * 100;
                      
                      const recommendations = [];
                      
                      if (riskLevel > 40) {
                        recommendations.push("🚨 Immediate intervention required - conduct stay interviews");
                      }
                      if (deptData.avgSatisfaction < 6) {
                        recommendations.push("📈 Focus on improving job satisfaction through better communication");
                      }
                      if (deptData.highRisk > 0) {
                        recommendations.push("⭐ Develop retention plans for high-risk employees");
                      }
                      if (riskLevel > 25) {
                      ...
                      return recommendations.map((rec, idx) => (
                        <div key={idx} className="text-sm text-yellow-800">{rec}</div>
                      ));
                    })()}
                  </div>
                </div>
                ...
                <div className="bg-green-50 rounded-lg p-4">
                  <h5 className="font-medium text-green-900 mb-3">Tenure Analysis</h5>
                  <div className="text-sm text-green-800">
                    <div>Average Tenure: {
                      (selectedDeptEmployees.reduce((sum, emp) => sum + (emp.tenure || 0), 0) / selectedDeptEmployees.length).toFixed(1)
                    } years</div>
                    <div>New Hires (&lt;1 year): {
                      selectedDeptEmployees.filter(emp => (emp.tenure || 0) < 1).length
                    }</div>
                    <div>Veterans (&gt;5 years): {
                      selectedDeptEmployees.filter(emp => (emp.tenure || 0) > 5).length
                    }</div>
                  </div>
                </div>
              </div>
        ...
        <div className="overflow-x-auto">
          <table className="min-w-full divide-y divide-gray-200">
            ...
            <tbody className="bg-white divide-y divide-gray-200">
              {departmentStats
                .sort((a, b) => ((b.highRisk + b.mediumRisk) / b.totalEmployees) - ((a.highRisk + a.mediumRisk) / a.totalEmployees))
                .map((dept, index) => {
                  const riskPercentage = ((dept.highRisk + dept.mediumRisk) / dept.totalEmployees) * 100;
                  return (
                    <tr key={index} className="hover:bg-gray-50">
                      ...
                      <td className="px-6 py-4 whitespace-nowrap text-sm text-gray-900">
                        {dept.highRisk} ({((dept.highRisk / dept.totalEmployees) * 100).toFixed(1)}%)
                      </td>
                      <td className="px-6 py-4 whitespace-nowrap text-sm text-gray-900">
                        {dept.avgSatisfaction}/10
                      </td>
                      <td className="px-6 py-4 whitespace-nowrap">
                        <span className={`inline-flex px-2 py-1 text-xs font-semibold rounded-full ${
                          riskPercentage < 20 ? 'bg-green-100 text-green-800' :
                          riskPercentage < 35 ? 'bg-yellow-100 text-yellow-800' :
                          'bg-red-100 text-red-800'
                        }`}>
                          {riskPercentage < 20 ? 'Healthy' :
                           riskPercentage < 35 ? 'Monitor' : 'Critical'}
                        </span>
                      </td>
                    </tr>
                  );
