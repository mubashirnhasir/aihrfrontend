    ...
    try {
        ...
        if (!hasRequiredSections) {
            return NextResponse.json(
                { message: 'Personal information and address are required for onboarding' },
                { status: 400 }
            );
        }
        ...
        console.log('🚀 Forwarding to backend:', `${BACKEND_URL}/api/employee/onboarding`);
...
export async function GET(request) {
    try {
        ...
        const response = await fetch(`${BACKEND_URL}/api/employees/onboarding-status`, {
            method: 'GET',
            headers: {
                'Authorization': authHeader,
                'Content-Type': 'application/json'
