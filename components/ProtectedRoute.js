...
const ProtectedRoute = ({ children }) => {
  ...
  useEffect(() => {
    ...
    const shouldWaitForLoading =
      (isEmployeeRoute && employeeLoading) || (isAdminRoute && adminLoading);

    if (!shouldWaitForLoading) {
      // Define public paths
      const adminPublicPaths = ["/signin", "/signup"];
      const employeePublicPaths = [
      ...
      const isPublicPath = allPublicPaths.some((path) =>
      ...
      if (isEmployeeRoute) {
        ...
        if (!isEmployeeAuthenticated && !isPublicPath) {
          ...
          (pathname === "/employee/auth/signin" ||
            pathname === "/employee/auth/signup")
        ) {
